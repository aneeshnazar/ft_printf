...
/*   Updated: 2017/08/17 14:23:28 by anazar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int	ft_intlen(unsigned char n, int base)
{
	int		i;
...
static char	get_digit(unsigned char n)
{
	if (n <= 9)
		return (n + '0');
	else
...
static void	help(char *buffer, int base, unsigned char num, int index)
{
...
char		*ft_hhutoa(unsigned char n, char base)
{
	int		len;
	char	*buffer;
	int		b;
